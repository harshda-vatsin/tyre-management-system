...
router.post('/', authorize(...WRITE_ROLES), asyncHandler(async (req, res) => {
  ...
  try {
    const info = await db.prepare('INSERT INTO packages (name, code) VALUES (?, ?)').run(name, code);
    const created = await db.prepare('SELECT * FROM packages WHERE id = ?').get(info.lastInsertRowid);

    await writeAuditLog({ user: req.user, action: 'CREATE', entityType: 'package', entityId: created.id, after: created });
...
router.put('/:id', authorize(...WRITE_ROLES), asyncHandler(async (req, res) => {
  const before = await db.prepare('SELECT * FROM packages WHERE id = ?').get(req.params.id);
  ...
  try {
    await db.prepare(`UPDATE packages SET name = ?, code = ?, updated_at = ${NOW_SQL} WHERE id = ?`).run(name, code, req.params.id);
  } catch (err) {
    if (err.code === PG_ERRORS.UNIQUE_VIOLATION) {
      return res.status(409).json({ error: 'Package code already exists' });
    }
  ...
  const after = await db.prepare('SELECT * FROM packages WHERE id = ?').get(req.params.id);
  await writeAuditLog({ user: req.user, action: 'UPDATE', entityType: 'package', entityId: after.id, before, after });
...
router.patch('/:id/status', authorize(...WRITE_ROLES), asyncHandler(async (req, res) => {
  ...
  await db.prepare(`UPDATE packages SET is_active = ?, updated_at = ${NOW_SQL} WHERE id = ?`).run(is_active ? 1 : 0, req.params.id);
